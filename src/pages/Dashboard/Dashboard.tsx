...
const Dashboard: FC = () => {
  ...
  function getData() {
    building
      .getBuildingAPI()
      .then((res: any) => setBuildingData({ data: res?.data['buildings'] }));
    energyConsumptionApi
      .getEnergyConsumptionAPI(startDate, stopDate, 15)
      .then((res: any) => setEnergyConsumptionData(res?.data['results']))
      .catch(() => setEnergyConsumptionData([]));
  }
      ...
      Object.keys(histData).forEach((val: any, index: number) => {
        if (val === 'today_power_kw') {
          histData['today_power_kw'].forEach((element: any) => {
            tmpPower.push({
              x: moment(moment(element['datetime']).format()).format(
                'YYYY-MM-DD HH:mm:ss'
              ),
              y: element['power']
            });
        ...
        if (val === 'zone_energy_percent') {
          Object.entries(histData['zone_energy_percent']['by_zone']).forEach(
        ...
        if (val === 'daily_energy_consumption') {
          Object.entries(histData['daily_energy_consumption']['value']).forEach(
            (element: any) => {
        ...
        if (val === 'floor_rank') {
          Object.entries(histData['floor_rank']['cu_terrace']['rank']).forEach(
            (element: any) => {
          ...
          Object.entries(histData['floor_rank']['cu_ihouse']['rank']).forEach(
            (element: any) => {
  ...
  return (
    <div className='row' style={{ marginLeft: '84px' }}>
          ...
          <div className='row'>
            <div className='column3'>
              ...
              <EnergyConsumptionByFloor
                cuTerrace={cuTerraceRank}
                cuiHouse={cuiHouseRank}
              />
      ...
      <div className='column2' style={{ padding: '0px 8px 8px 20px' }}>
        ...
        <WeatherOutDoor dashboardData={dashboardData} />
